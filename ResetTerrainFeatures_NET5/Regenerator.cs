	...
	public static class Regenerator
	{
		...
		public static void clear(GameLocation location, Type[] types = null, int[] parentSheetIndices = null)
		{
			...
			foreach (Vector2 key2 in location.objects.Keys)
			{
				...
				bool flag3 = types == null || types.Contains(@object.GetType()) || (parentSheetIndices != null && parentSheetIndices.Contains(@object.ParentSheetIndex)) || types.Contains(typeof(StardewValley.Object)) || (Regenerator.regeneratorOptions.ContainsKey("Forage") && Regenerator.regeneratorOptions["Forage"] && @object.isForage(location));
				if (flag3)
				{
					list2.Add(key2);
				}
		...
		public static void loadMapFeatures(GameLocation location, Type[] types = null, int[] parentSheetIndices = null)
		{
			...
			{
				bool flag2 = (types == null || types.Contains(pair2.Value.GetType()) || (parentSheetIndices != null && parentSheetIndices.Contains(pair2.Value.ParentSheetIndex)) || types.Contains(typeof(StardewValley.Object)) || (Regenerator.regeneratorOptions.ContainsKey("Forage") && Regenerator.regeneratorOptions["Forage"] && pair2.Value.isForage(location))) && !location.terrainFeatures.ContainsKey(pair2.Key) && !location.objects.ContainsKey(pair2.Key);
				if (flag2)
				{
			...
			if (!flag4)
			{
				foreach (ResourceClump item in clumps)
				{
					...
					if (flag5)
					{
						(location as Farm).addResourceClumpAndRemoveUnderlyingTerrain(item.parentSheetIndex.Value, item.width.Value, item.height.Value, item.tile.Value);
					}
				}
			}
		}
